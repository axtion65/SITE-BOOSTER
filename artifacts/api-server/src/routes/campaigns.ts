...
router.get("/campaigns/:id/video-brief",async(req,res)=>{const userId=await owner(req,res);if(!userId)return;const row=(await pool.query(`SELECT vb.* FROM campaign_video_briefs vb JOIN campaigns c ON c.id=vb.campaign_id AND c.user_id=vb.customer_id WHERE vb.campaign_id=$1 AND vb.customer_id=$2`,[req.params.id,userId])).rows[0];if(!row){res.status(404).json({error:"Campaign video preparation has not been created.",requestId:(req as any).id});return;}res.json(row);});
