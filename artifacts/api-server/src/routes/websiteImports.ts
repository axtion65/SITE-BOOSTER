...
router.post('/website-imports/:id/approve',async(req,res)=>{
  ...
  try{
    ...
    await client.query("INSERT INTO campaigns(id,user_id,business_id,product_id,name,brief,source_url,website_import_id,context_snapshot,identity_resolution) VALUES($1,$2,$3,NULL,$4,$5,$6,$7,$8,$9)",[campaignId,userId,business.id,parsed.data.campaign.name,{objective:parsed.data.campaign.objective,campaignType:parsed.data.campaign.campaignType,channel:parsed.data.campaign.channel,promotion:parsed.data.campaign.promotion||'',instructions:`Website profile approved from ${c.business.website}`},draft.source_url,draft.id,contextSnapshot,identityResolution]);
