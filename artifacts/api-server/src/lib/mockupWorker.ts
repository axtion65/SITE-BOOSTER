...
async function saveAsset(job:Job){
  ...
  try{
    await pool.query(`UPDATE mockup_versions SET object_path=$2,status=$3,job_stage=$3,qa_decision=$3,qa_checks=$4,width=$5,height=$6,content_type=$7,lease_owner=NULL,lease_expires_at=NULL WHERE id=$1`,
    ...
    await pool.query("UPDATE mockup_projects SET status=$2,updated_at=NOW() WHERE id=$1",[job.mockup_project_id,qa.decision]);
