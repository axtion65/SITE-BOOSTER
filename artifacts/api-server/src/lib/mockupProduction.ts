...
export function isRecoverableDimensionMetadataFailure(version:{status?:string;object_path?:string|null;qa_checks?:Record<string,boolean>|null}){const checks=version.qa_checks||{};return version.status==="needs_revision"&&Boolean(version.object_path)&&checks.expectedDimensions===false&&Object.entries(checks).every(([name,passed])=>name==="expectedDimensions"||passed===true);}
