...
test("Quae-choice creation reuses a persisted pending candidate set", () => {
  ...
  assert.match(createRoute, /archetype='quae_choice'.*jsonb_array_length\(reference_object_paths\)>1/);
