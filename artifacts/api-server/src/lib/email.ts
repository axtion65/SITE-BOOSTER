...
async function queueEmail(to: string, toName: string, subject: string, html: string) {
  ...
  try {
    ...
    logger.error({ event: "email.queue_failed", ...safeErrorMetadata(err) }, "Email could not be queued");
...
async function markQueued(id: string, status: "sent" | "failed" | "pending" | "processing", error?: string) {
  ...
  try {
    ...
    logger.error({ event: "email.queue_status_failed", queueId: id, ...safeErrorMetadata(err) }, "Email queue status could not be saved");
  }
}
...
async function sendViaResend(
  ...
  // quae.ai domain is verified on Resend — send from noreply@quae.ai.
