...
test("campaign and standalone scripts preview the complete production plan", () => {
  assert.equal(compilePreviewRenderBrief(script,30,5).visualBeats.length,4);
  assert.equal(compilePreviewRenderBrief(script,30,10).visualBeats.length,4);
  assert.equal(compilePreviewRenderBrief(script,5,10).shortened,false);
  assert.equal(compilePreviewRenderBrief(script,30,5).renderDurationSeconds,30);
  assert.match(compilePreviewRenderBrief(script,30,5).marketingMessage,/\$10/);
  assert.match(compilePreviewRenderBrief(script,30,5).visualProductionBrief,/multi-scene business advertisement/i);
});
