...
import { Field, fieldClass, MarketingPage, PremiumCard, SaveButton, SectionHeading, StatusPill } from "./marketing-shared";
...
export default function BusinessPage() {
  ...
  return <MarketingPage eyebrow="Marketing foundation" title="Your business" description="Teach Quae the essentials once. We only use details you provide and never invent missing business information."><form onSubmit={save} className="space-y-6">
    <PremiumCard className="grid gap-5 md:grid-cols-2"><div className="md:col-span-2 flex items-start justify-between gap-4"><SectionHeading title="Business identity" description="The core details Quae uses to represent your company accurately."/><StatusPill>Learning</StatusPill></div>
    ...
    <Field label="Tagline"><input className={fieldClass} value={form.tagline ?? ""} onChange={(e) => set("tagline",e.target.value)} /></Field><Field label="Primary call-to-action"><input className={fieldClass} value={form.primaryCta ?? ""} onChange={(e) => set("primaryCta",e.target.value)} /></Field></PremiumCard>
    <PremiumCard className="grid gap-5 md:grid-cols-2"><div className="md:col-span-2"><SectionHeading title="Contact & location" description="Public details that can be used in customer-facing marketing."/></div>
    {[['website','Website','url'],['phone','Phone','text'],['publicEmail','Public email','email'],['streetAddress','Street address','text'],['city','City','text'],['region','State / region','text'],['country','Country','text']].map(([key,label,type]) => <Field key={key} label={label}><input type={type} className={fieldClass} value={form[key] ?? ""} onChange={(e) => set(key,e.target.value)} /></Field>)}</PremiumCard>
