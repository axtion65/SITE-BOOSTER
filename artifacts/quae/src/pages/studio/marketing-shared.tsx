...
export const fieldClass = "w-full rounded-xl border border-white/10 bg-[#1B2940] px-4 py-3 text-sm text-slate-50 shadow-inner shadow-slate-950/10 transition hover:border-violet-300/30 focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-500/25 disabled:bg-[#18263D]";
...
export function MarketingImage({ objectPath, className, alt = "" }: { objectPath: string; className?: string; alt?: string }) { const url = usePrivateImageUrl(privateImageUrl(objectPath)); return url ? <img src={url} alt={alt} className={className} /> : <div role="status" aria-label="Loading secure image" className={`${className ?? ""} animate-pulse bg-white/[.05]`} />; }
